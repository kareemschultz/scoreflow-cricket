...
import { computeDominoTeamStats } from "@/lib/dominoes-stats"
import { computeDominoTournamentStandings } from "@/lib/domino-tournaments"
import { seedDemoDominoData } from "@/lib/demo-seed"
...
function LeaderboardTable({ onLoadDemo, loadingDemo }: { onLoadDemo: () => void; loadingDemo: boolean }) {
  ...
  const stats = computeDominoTeamStats(teams, matches, players)

  return (
    <div className="overflow-x-auto -mx-4">
      <table className="w-full text-xs min-w-[480px] px-4">
        <thead>
          <tr className="text-muted-foreground border-b border-border">
            ...
            <th className="px-2 py-2 text-center font-medium">6-0</th>
        ...
        <motion.tbody initial="hidden" animate="visible" variants={containerVariants}>
          {stats.map((s, idx) => (
            ...
            >
              ...
              <td className="px-2 py-2.5 text-center tabular-nums">{s.sixLoves}</td>
...
function RecentMatches() {
  ...
  const data = useLiveQuery(async () => {
    const matches = await db.dominoMatches.orderBy("date").reverse().limit(5).toArray()
  ...
  return (
    <div className="space-y-2">
      {data.map((m) => {
        ...
        const isSixLove = (m.winnerId === m.team1Id && m.team2Score === 0) ||
              ...
              <CardContent className="py-3 px-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    {m.tournament && (
                      <p className="text-[10px] text-amber-500 font-medium mt-1">{m.tournament.name}</p>
                    )}
                  </div>
                </div>
              </CardContent>
...
function TournamentSnapshot() {
  ...
  const data = useLiveQuery(async () => {
    const tournaments = await db.dominoTournaments.toArray()
    const teams = await db.dominoTeams.toArray()
    const matches = await db.dominoMatches.toArray()

    return tournaments
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
      .slice(0, 3)
      .map((tournament) => {
        const standings = computeDominoTournamentStandings(tournament, teams, matches)
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center space-y-3">
          ...
          <Button variant="outline" size="sm" onClick={() => navigate({ to: "/dominoes/tournaments" })}>
            Open Tournaments
          </Button>
        </CardContent>
      </Card>
            ...
            <CardContent className="py-3 px-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] mt-1 text-amber-500 font-medium">
                    {champion
  ...
  return (
    <div className="min-h-full bg-background relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden>
        <motion.div
          className="absolute -top-20 -left-20 w-56 h-56 rounded-full bg-amber-500/5"
        ...
        {/* Domino tile SVG */}
        <motion.svg
          className="absolute top-20 right-6 opacity-[0.06]"
          width="40" height="80" viewBox="0 0 40 80"
          animate={{ rotate: [0, 5, -5, 0] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
        >
          <rect x="2" y="2" width="36" height="76" rx="4" fill="white" />
          <line x1="2" y1="40" x2="38" y2="40" stroke="#333" strokeWidth="2" />
          <circle cx="20" cy="20" r="3" fill="#333" />
          <circle cx="12" cy="52" r="3" fill="#333" />
          <circle cx="28" cy="52" r="3" fill="#333" />
          <circle cx="12" cy="68" r="3" fill="#333" />
          <circle cx="28" cy="68" r="3" fill="#333" />
        </motion.svg>
      </div>
...
export const Route = createFileRoute("/dominoes/")({
  component: DominoesHomePage,
})
