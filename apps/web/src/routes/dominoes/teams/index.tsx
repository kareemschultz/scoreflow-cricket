...
function AddPlayerDialog({
  ...
  async function handleSave() {
    ...
    setError(null)
    try {
      await db.dominoPlayers.add({ id: nanoid(), name: name.trim(), colorHex: color, createdAt: new Date() })
      onClose()
      setName("")
      ...
      <DialogContent showCloseButton>
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input id="pname" placeholder="e.g. Big Yard" value={name} onChange={(e) => setName(e.target.value)}
...
function AddTeamDialog({
  ...
  const players = useLiveQuery(() => db.dominoPlayers.orderBy("createdAt").toArray())
  const [name, setName] = useState("")
  const [color, setColor] = useState(PRESET_COLORS[0])
      ...
      <DialogContent showCloseButton>
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input placeholder="e.g. Road Warriors" value={name} onChange={(e) => setName(e.target.value)} autoFocus />
          </div>
...
function DominoTeamsPage() {
  ...
  const stats = teams && matches && players ? computeDominoTeamStats(teams, matches, players) : []
  const statsMap = new Map(stats.map((s) => [s.teamId, s]))
  const playerMap = new Map((players ?? []).map((p) => [p.id, p]))

  async function handleDelete() {
    if (!deleteId) return
    await db.dominoTeams.delete(deleteId)
    setDeleteId(null)
  }
                        ...
                        <CardContent className="py-3 px-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {s && s.matchesPlayed > 0 && (
                                <p className="text-[10px] text-muted-foreground">
                                  {s.matchesPlayed} played &middot; {s.matchesWon}W {s.matchesLost}L &middot; {s.winRate}%
                                </p>
                              )}
                            </div>
...
export const Route = createFileRoute("/dominoes/teams/")({
  component: DominoTeamsPage,
})
