...
export function BatsmanBreakdownChart({ innings, teamNames }: BatsmanBreakdownChartProps) {
  const inningsData = innings.map((inn) => {
    const batsmen = inn.battingCard
      .filter((b) => b.balls >= 5 && b.runs > 0)
      .map((b) => {
        const other = Math.max(0, b.balls - b.dots - b.fours - b.sixes)
        return {
          name: b.playerName,
          runs: b.runs,
          dots: b.dots,
      ...
      {inningsData.map((batsmen, idx) => {
        ...
        return (
          <div key={idx} className="space-y-2">
            ...
            {batsmen.map((b, i) => {
              const maxBalls = Math.max(...batsmen.map((x) => x.total), 1)
              const scaleFactor = b.total / maxBalls

              const dotPct = b.total > 0 ? (b.dots / b.total) * 100 * scaleFactor : 0
              const otherPct = b.total > 0 ? (b.other / b.total) * 100 * scaleFactor : 0
              const fourPct = b.total > 0 ? (b.fours / b.total) * 100 * scaleFactor : 0
              const sixPct = b.total > 0 ? (b.sixes / b.total) * 100 * scaleFactor : 0
